...
suf = sorted({n.split('.')[-1] for n, mod in m.named_modules()
             if mod.__class__.__name__ in ('Linear4bit', 'Linear', 'Linear8bitLt')})
print("RESULT phi_linear_suffixes", suf)
print("RESULT phi_load_VRAM_GB", round(torch.cuda.memory_allocated()/1e9, 2))
m.config.use_cache = False
lora = LoraConfig(task_type="CAUSAL_LM", r=16, lora_alpha=32, lora_dropout=0.05,
    target_modules=["qkv_proj", "o_proj", "gate_up_proj", "down_proj"])
args = SFTConfig(output_dir="outputs/_psmoke", max_length=384, per_device_train_batch_size=1,
    gradient_accumulation_steps=16, max_steps=8, logging_steps=4, learning_rate=2e-4, bf16=True,
...
r = tr.train()
print("RESULT phi_train_loss", round(r.training_loss, 4),
...
print("RESULT phi_FT_48ex", {k: round(v, 3) for k, v in met.items()})
print("RESULT phi_eval_VRAM_GB", round(torch.cuda.max_memory_allocated()/1e9, 2))
print("RESULT PHI_OK")
